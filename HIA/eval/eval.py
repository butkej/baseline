...
def CalculatePatientWiseAUC(
    resultCSVPath, args, reportFile, foldcounter=None, clamMil=False
):
    ...
    for index, key in enumerate(keys):
        ...
        for patient in patients:
            ...
            yTrueLabelList.append(
                utils.get_key_from_value(args.target_labelDict, data_temp["yTrue"][0])
            )
            if not clamMil:
                dl_pred = np.where(
                    data_temp[keys_temp].lt(data_temp[key], axis=0).all(axis=1),
                    True,
                    False,
                )
        ...
        fpr, tpr, thresholds = metrics.roc_curve(
            yTrueList, yProbList, pos_label=args.target_labelDict[key]
        )
        if foldcounter:
            print(
                "\nAUC FOR TARGET {} IN THIS DATA SET IN FOLD {} IS: {} ".format(
                    key, foldcounter, np.round(metrics.auc(fpr, tpr), 3)
                )
            )
            reportFile.write(
                "AUC FOR TARGET {} IN THIS DATA SET IN FOLD {} IS: {} ".format(
            ...
            path = os.path.join(
                args.result_dir,
                "TEST_RESULT_PATIENT_BASED_FOLD_" + str(foldcounter) + ".csv",
            )
        else:
            print(
                "\nAUC FOR TARGET {} IN THIS DATA SET IS: {} ".format(
                    key, np.round(metrics.auc(fpr, tpr), 3)
                )
            )
            reportFile.write(
                "AUC FOR TARGET {} IN THIS DATA SET IS: {} ".format(
    ...
    df = pd.DataFrame(
        list(zip(patientsList, yTrueList, yTrueLabelList)),
...
def CalculateTotalROC(resultsPath, results, target_labelDict, reportFile):
    ...
    for key in keys:
        ...
        fpr, tpr, thresholds = metrics.roc_curve(
            y_true, y_pred, pos_label=target_labelDict[key]
        )
        print("-" * 30)
        print(
            "TOTAL AUC FOR target {} IN THIS DATASET IS : {} ".format(
                key, np.round(metrics.auc(fpr, tpr), 3)
            )
        )
        reportFile.write("-" * 30 + "\n")
        ...
        for i in range(nsamples):
            ...
            if len(np.unique(y_pred[indices])) < 2 or np.sum(y_true[indices]) == 0:
                continue
            fpr, tpr, thresholds = metrics.roc_curve(
                y_true[indices], y_pred[indices], pos_label=target_labelDict[key]
            )
        ...
        auc_values.sort()
        reportFile.write(
            "Lower Confidebnce Interval For Target {}: {}".format(
                key, np.round(auc_values[int(0.025 * len(auc_values))], 3)
            )
            + "\n"
        )
        reportFile.write(
            "Higher Confidebnce Interval For Target {} : {}".format(
                key, np.round(auc_values[int(0.975 * len(auc_values))], 3)
            )
            + "\n"
        )
        print(
    ...
    totalData.to_csv(
        os.path.join(resultsPath, "TEST_RESULT_PATIENT_BASED_TOTAL.csv"), index=False
    )
...
def MergeResultCSV(resultsPath, results, milClam=False):
    ...
    if milClam:
        totalData.to_csv(
            os.path.join(resultsPath, "TEST_RESULT_SLIDE_BASED_TOTAL.csv"), index=False
        )
    else:
        totalData.to_csv(
            os.path.join(resultsPath, "TEST_RESULT_TILE_BASED_TOTAL.csv"), index=False
        )
...
def GenerateHighScoreTiles_Classic(
    totalPatientResultPath,
    totalResultPath,
    numHighScorePetients,
    numHighScoreTiles,
    target_labelDict,
    savePath,
):
...
def GenerateHighScoreTiles(
    ...
    for key in keys:
        ...
        for patient in highScorePosPatients:
            temp_scores = tileScoresData[
                tileScoresData["FILENAME"].str.contains(patient)
            ]
            ...
            temp = pd.DataFrame(
                list(zip(scores, coord_x, coord_y)), columns=[key, "coord_x", "coord_y"]
            )
            ...
            tilePaths = glob.glob(
                os.path.join(imgsPath[0], temp_scores.iloc[0, 0].split("*")[-1])
            )[0]
            ...
            for item in range(len(coordx)):
                try:
                    doFind = [
                        v
                        for v in imgs
                        if "(" + str(coordx[item]) + "," + str(coordy[item]) + ")" in v
                    ]
                    img = Image.open(doFind[0])
                    copyfile(
                        doFind[0],
                        os.path.join(
                            os.path.join(savePath, key), doFind[0].split("\\")[-1]
                        ),
                    )
