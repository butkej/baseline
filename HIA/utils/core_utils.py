...
def Train_MIL_CLAM(datasets, fold, args, trainFull=False):
    ...
    train_loader = Get_split_loader(
        split_dataset=train_split, training=True, weighted=args.weighted_sample
    )
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(
            patience=args.patience, stop_epoch=args.minEpochToTrain, verbose=True
        )
    ...
    if (args.early_stopping) and (not trainFull):
        model.load_state_dict(
            torch.load(os.path.join(args.result_dir, "bestModelFold_" + str(fold)))
        )
    if not trainFull:
        patient_results, test_error, test_auc = Summary(
            model=model,
            loader=test_loader,
            n_classes=args.num_classes,
...
def validate_CLAM(
    ...
    if inst_count > 0:
        ...
        for i in range(2):
            acc, correct, count = inst_logger.get_summary(i)
            print(
                "clustering: class {}, acc {}, correct {}/{}".format(
                    i, acc, correct, count
                )
            )
    for i in range(n_classes):
    ...
    if n_classes == 2:
        for i in range(n_classes):
            fpr, tpr, thresholds = metrics.roc_curve(
                all_labels, all_probs[:, i], pos_label=i
            )
            auc = metrics.auc(fpr, tpr)
            aucs = []
    else:
...
def Train_model_Classic(
    model, trainLoaders, args, valLoaders=[], criterion=None, optimizer=None, fold=False
):
