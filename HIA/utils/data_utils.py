...
def Generate_batch(image_file_list, status_list, maxBlockNum):
    ...
    for index, each_path in enumerate(image_file_list):
        ...
        if len(image_pathContent) > maxBlockNum:
            image_pathContent = np.random.choice(
                image_pathContent, maxBlockNum, replace=False
            )
...
def Get_train_valid_Path(trainSet, train_percentage=0.8):
    ...
    train_index, test_index = np.asarray(indexes[:num_train]), np.asarray(
        indexes[num_train:]
    )
    ...
    if training:
        if weighted:
            ...
            loader = DataLoader(
                ...
                collate_fn=collate_MIL_Training,
                **kwargs
            )
        else:
            loader = DataLoader(
        ...
        loader = DataLoader(
            ...
            collate_fn=collate_MIL_Testing,
            **kwargs
        )
    return loader
...
def ConcatCohorts_Classic(
    ...
    for imgCounter in range(len(imagesPath)):
        ...
        print(
            "{} Patients didnt have the proper label for target label: {}\n".format(
                lenBefore - lenafter, label
            )
        )
        reportFile.write(
            "{} Patients didnt have the proper label for target label: {}".format(
                lenBefore - lenafter, label
            )
            + "\n"
        )
        ...
        print(
            "Data for {} Patients from Clini Table is not found in Slide Table!\n".format(
                len(inClinicalNotInSlide)
            )
        )
        reportFile.write(
            "Data for {} Patients from Clini Table is not found in Slide Table!".format(
                len(inClinicalNotInSlide)
            )
            + "\n"
        )
        ...
        print(
            "Data for {} Patients from Slide Table is not found in Clini Table!\n".format(
                len(inSlideNotInClinical)
            )
        )
        reportFile.write(
            "Data for {} Patients from Slide Table is not found in Clini Table!".format(
                len(inSlideNotInClinical)
            )
            + "\n"
        )
    ...
    if patientNumber == "ALL":
        for patientID in tqdm(patients):
            indicies = [
                i for i, n in enumerate(slideTable_PatintNotUnique) if n == patientID
            ]
            ...
            for slide in matchedSlides:
                ...
                if not len(slideName) == 0:
                    ...
                    reportFile.write(
                        "Slide {} is dropped out because of Pre-Processing.".format(
                            slide
                        )
                        + "\n"
                    )
        ...
        while len(patientList_return) < int(patientNumber):
            ...
            indicies = [
                i
                for i, n in enumerate(slideTable_PatintNotUnique)
                if n == samplePatient
            ]
            matchedSlides = [list(slideTableList["FILENAME"])[i] for i in indicies]

            temp = clinicalTableList.loc[
                clinicalTableList["PATIENT"] == str(samplePatient)
            ]
            temp.reset_index(drop=True, inplace=True)
    ...
    return (
        patientList_return,
        labelList_return,
        os.path.join(outputPath, csvName + ".csv"),
    )
...
def GetTiles(
    ...
    for index, patientID in enumerate(tqdm(patientsUnique)):
        ...
        for item in range(len(selectedData)):
            tempTiles.extend(
                [
                    os.path.join(selectedData["SlideAdr"][item], i)
                    for i in os.listdir(selectedData["SlideAdr"][item])
                ]
            )
        ...
        for tile in tempTiles:
            ...
            yTrueList.append(
                utils.get_value_from_key(target_labelDict, selectedData[label][0])
            )
            yTrueLabelList.append(selectedData[label][0])
            patinetList.append(str(patientID))

    df = pd.DataFrame(
        list(zip(patinetList, tilesPathList, yTrueList, yTrueLabelList)),
        columns=["PATIENT", "TilePath", "yTrue", "yTrueLabel"],
    )
    df_temp = df.dropna()
