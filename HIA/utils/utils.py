...
def CreateProjectFolder(ExName, ExAdr, targetLabel, model_name, repeat=None):
    ...
    if repeat:
        outputPath = os.path.join(
            outputPath_root, ExName + "_" + targetLabel + "_" + str(repeat)
        )
    else:
...
def RenameTCGASLideNamesInSlideTable(slideTablePath, imgsFolder):
    ...
    if slideTablePath.split(".")[-1] == "csv":
        slideTable = pd.read_csv(
            slideTablePath, sep=r"\s*,\s*", header=0, engine="python"
        )
    else:
...
def ReadExperimentFile(args, deploy=False):
    ...
    args.csv_name = "TCGA-CRC-DX_SLIDE.csv"
    ...
    if not deploy:
        for index, item in enumerate(datadir_train):
            ...
            if not deploy:
                if os.path.isfile(
                    os.path.join(item, item.split("/")[-1] + "_CLINI.xlsx")
                ):
                    args.clini_dir.append(
                        os.path.join(item, item.split("/")[-1] + "_CLINI.xlsx")
                    )
                else:
                ...
                if os.path.isfile(
                    os.path.join(item, item.split("/")[-1] + "_SLIDE.csv")
                ):
                    args.slide_dir.append(
                        os.path.join(item, item.split("/")[-1] + "_SLIDE.csv")
                    )
                else:
    ...
    try:
        ...
        print(
            "NUMBER OF PATIENTS TO USE IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : ALL\n"
        )
        print("-" * 30)
    ...
    try:
        ...
        print(
            "NUMBER OF K FOLD CROSS ENTROPY IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 3\n"
        )
        print("-" * 30)
        args.k = 3
    ...
    try:
        ...
        print(
            "DECREASE RATE OF LR IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 0.00001\n"
        )
        print("-" * 30)
        args.reg = 0.00001
    try:
    ...
    if deploy:
        try:
            ...
            print(
                "NUMBER OF HIGH SCORE PATIENTS IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 5\n"
            )
            print("-" * 30)
        ...
        try:
            ...
            print(
                "NUMBER OF HIGH SCORE TILES IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 5\n"
            )
            print("-" * 30)
            args.numHighScoreTiles = 5

    if (
        args.model_name == "clam_sb"
        or args.model_name == "clam_mb"
        or args.model_name == "mil"
        or args.model_name == "attmil"
    ):
    ...
    try:
        ...
        print(
            "REPEAT EXPERIEMNT NNUmBER IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : 1\n"
        )
        print("-" * 30)
    ...
    try:
        ...
        print(
            "EARLY STOPIING VALUE IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : TRUE\n"
        )
        print("-" * 30)
    ...
    if args.early_stopping:
        try:
            ...
            print(
                "MIN NUMBER OF EPOCHS TO TRAIN IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : 10\n"
            )
            print("-" * 30)
        ...
        try:
            ...
            print(
                "PATIENCE VALUE FOR EARLY STOPPING IS NOT DEFINED!\n DEFAULT VALUE WILL BE USED : 10\n"
            )
            print("-" * 30)
            args.patience = 10
    ...
    if not args.model_name in ["clam_sb", "clam_mb", "mil", "attmil"]:
        ...
        try:
            ...
            print(
                "MAX NUMBER OF BLOCKS IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 200\n"
            )
            print("-" * 30)
    ...
    if args.model_name in ["clam_sb", "clam_mb", "mil"]:
        ...
        try:
            ...
            print(
                "WEIGHTED SAMPLE VALUE IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : FALSE\n"
            )
            print("-" * 30)
    ...
    if args.model_name in ["clam_sb", "clam_mb", "mil", "attmil"]:
        try:
            ...
            print(
                "EXTRACT FEATURE VALUE IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : FALSE\n"
            )
            print("-" * 30)
    ...
    if args.model_name in ["clam_sb", "clam_mb"]:
        ...
        try:
            ...
            print(
                "VALUE OF SAMPLES IN A BAG IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : 8\n"
            )
            print("-" * 30)
            args.B = 8
        ...
        try:
            ...
            print(
                "NO INSTANCE CLUSTER IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : FALSE\n"
            )
            print("-" * 30)
        ...
        try:
            ...
            print(
                "EXTRACT Subtyping VALUE IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : FALSE\n"
            )
            print("-" * 30)
...
def CheckForTargetType(labelsList):
    ...
    if len(set(labelsList)) >= 5:
        ...
        if not len(checkList1) == 0 or not len(checkList2):
            ...
            raise NameError(
                "IT IS NOT POSSIBLE TO BINARIZE THE NOT NUMERIC TARGET LIST!"
            )
    return labelsList
