...
parser.add_argument(
    "--adressExp",
    type=str,
    default=r"L:\Experiments\TCGA_CLAM_TESTFULL.txt",
    help="Adress to the experiment File",
)
parser.add_argument(
    "--modelAdr",
    type=str,
    default=r"L:\Experiments\DACHS_CLAM_TRAINFULL_isMSIH_1\RESULTS\bestModel",
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.projectFolder):
        print(
            "THIS FOLDER IS ALREADY EXITS!!! PLEASE REMOVE THE FOLDER, IF YOU WANT TO RE-RUN."
        )
    else:
    ...
    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
        from_id=False, csv_path=os.path.join(args.split_dir, "TestSplit.csv")
    )
    ...
    for item in list(testPatientResults.keys()):
        ...
        yTrueLabe_test.append(
            utils.get_key_from_value(args.target_labelDict, temp["label"])
        )
        ...
        for key in list(args.target_labelDict.keys()):
            probs[key].append(
                temp["probs"][utils.get_value_from_key(args.target_labelDict, key)]
            )
    ...
    totalPatientResultPath = CalculatePatientWiseAUC(
    ...
    GenerateHighScoreTiles(
        imgsPath=args.datadir_test,
        totalPatientResultPath=totalPatientResultPath,
        totalResultPath=testResultsPath,
        tileScorePath=os.path.join(args.result_dir, "TileScores.csv"),
        coordsPath=os.path.join(args.result_dir, "Coordinates.csv"),
        numHighScorePetients=args.numHighScorePatients,
        numHighScoreTiles=args.numHighScorePatients,
