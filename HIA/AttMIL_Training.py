    ...
    for targetLabel in targetLabels:
        for repeat in range(args.repeatExperiment):
            ...
            if args.train_full:
                ...
                val_data = train_data.groupby(
                    args.target_label, group_keys=False
                ).apply(lambda x: x.sample(frac=0.1))
                train_data["is_valid"] = train_data.PATIENT.isin(val_data["PATIENT"])
                train_data["SlideAdr"] = [
                    i.replace("BLOCKS_NORM_MACENKO", "FEATURES")
                    for i in train_data["SlideAdr"]
                ]
                train_data["SlideAdr"] = [
                    Path(i + ".pt") for i in train_data["SlideAdr"]
                ]
                train_data.to_csv(
                ...
                dblock = DataBlock(
                    ...
                    item_tfms=MILBagTransform(
                        train_data[train_data.is_valid].SlideAdr, 4096
                    ),
                )
                dls = dblock.dataloaders(train_data, bs=args.batch_size)
                weight = (
                    train_data[args.target_label].value_counts().sum()
                    / train_data[args.target_label].value_counts()
                )
                weight /= weight.sum()
                ...
                history.to_csv(
                    os.path.join(args.result_dir, "TRAIN_HISTORY_FULL" + ".csv"),
                    index=False,
                )
                print()
                print("-" * 30)
                ...
                for train_index, test_index in kf.split(patientID, yTrue):
                    ...
                    train_data["is_valid"] = train_data.PATIENT.isin(
                        val_data["PATIENT"]
                    )
                    train_data["SlideAdr"] = [
                        i.replace("BLOCKS_NORM_MACENKO", "FEATURES")
                    ...
                    test_data["SlideAdr"] = [
                        i.replace("BLOCKS_NORM_MACENKO", "FEATURES")
                        for i in test_data["SlideAdr"]
                    ]
                    test_data["SlideAdr"] = [
                        Path(i + ".pt") for i in test_data["SlideAdr"]
                    ]
                    test_data.to_csv(
                        os.path.join(args.split_dir, "TestSplit.csv"), index=False
                    )
                    ...
                    torch.save(
                        ...
                        os.path.join(
                            args.projectFolder,
                            "RESULTS",
                            "finalModelFold" + str(foldcounter),
                        ),
                    )
                    history = pd.DataFrame(
                        list(
                    ...
                    history.to_csv(
                        os.path.join(
                            args.result_dir,
                            "TRAIN_HISTORY_FOLD_" + str(foldcounter) + ".csv",
                        ),
                        index=False,
                    )
                    ...
                    test_data = test_data.rename(
                        columns={args.target_label: "yTrueLabel"}
                    )
                    test_data["yTrue"] = [
                        utils.get_value_from_key(args.target_labelDict, i)
                        for i in test_data["yTrueLabel"]
                    ]
                ...
                for i in range(args.k):
                    ...
                    tileScoreFiles.append(
                        "TEST_RESULT_TILE_BASED_FOLD_" + str(i + 1) + ".csv"
                    )
                CalculateTotalROC(
