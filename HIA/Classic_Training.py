        ...
        for repeat in range(args.repeatExperiment):
            ...
            if args.train_full:
                ...
                if args.early_stopping:
                    ...
                    train_y = list(train_data["yTrue"])
                    val_data.to_csv(
                        os.path.join(args.split_dir, "ValSplit.csv"), index=False
                    )
                else:
                    valGenerator = []
                ...
                train_set = DatasetLoader_Classic(
                    train_x,
                    train_y,
                ...
                (
                    ...
                    model=model,
                    trainLoaders=trainGenerator,
                    valLoaders=valGenerator,
                    criterion=criterion,
                    optimizer=optimizer,
                    args=args,
                    fold="FULL",
                ...
                for train_index, test_index in kf.split(patientID, labels):
                    ...
                    test_data.to_csv(
                        os.path.join(
                            args.split_dir, "TestSplit_" + str(foldcounter) + ".csv"
                        ),
                        index=False,
                    )
                    train_data.to_csv(
                        os.path.join(
                            args.split_dir, "TrainSplit_" + str(foldcounter) + ".csv"
                        ),
                        index=False,
                    )
                    val_data.to_csv(
                        os.path.join(
                            args.split_dir, "ValSplit_" + str(foldcounter) + ".csv"
                        ),
                        index=False,
                    )
                    ...
                    test_set = DatasetLoader_Classic(
                        test_x,
                        test_y,
                        transform=torchvision.transforms.ToTensor,
                        target_patch_size=input_size,
                    ...
                    probsList = Validate_model_Classic(
                        model=model, dataloaders=testGenerator
                    )
                    ...
                    testResultsPath = os.path.join(
                        args.result_dir,
                        "TEST_RESULT_TILE_BASED_FOLD_" + str(foldcounter) + ".csv",
                    )
