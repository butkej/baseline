...
class Generic_WSI_Classification_Dataset(Dataset):
    def __init__(
        ...
        df_temp = pd.DataFrame(
            list(zip(patients, patient_labels)), columns=["PATIENT", "label"]
        )
        ...
        self.patient_data = {
            "PATIENT": np.array(df_temp["PATIENT"]),
            "label": np.array(df_temp["label"]),
        }
        self.Cls_ids_prep()
    ...
    def Cls_ids_prep(self):
        ...
        for i in range(self.num_classes):
            self.patient_cls_ids[i] = np.where(
                np.array(self.patient_data["label"]) == i
            )[0]
        ...
        if from_id:
            if len(self.train_ids) > 0:
                ...
                train_split = Generic_Split(
                    train_data, data_dir=self.data_dir, num_classes=self.num_classes
                )
            else:
                train_split = None
            ...
            if len(self.val_ids) > 0:
                ...
                val_split = Generic_Split(
                    val_data, data_dir=self.data_dir, num_classes=self.num_classes
                )
            else:
                val_split = None
            ...
            if len(self.test_ids) > 0:
                ...
                test_split = Generic_Split(
                    test_data, data_dir=self.data_dir, num_classes=self.num_classes
                )
            else:
                test_split = None
