...
def CLAM_MIL_Training(args):
    ...
    for targetLabel in targetLabels:
        for repeat in range(args.repeatExperiment):
            ...
            reportFile = open(
                os.path.join(args.projectFolder, "Report.txt"), "a", encoding="utf-8"
            )
            reportFile.write("-" * 30 + "\n")
            ...
            patientsList, labelsList, args.csvFile = ConcatCohorts_Classic(
            ...
            if args.train_full:
                ...
                train_data = pd.DataFrame(
                    list(zip(patientsList, yTrue, yTrueLabel)),
                    columns=["PATIENT", "yTrue", "yTrueLabel"],
                )
                if args.early_stopping:
                    val_data = train_data.groupby("yTrue", group_keys=False).apply(
                        lambda x: x.sample(frac=0.1)
                    )
                    train_data = train_data[
                        ~train_data["PATIENT"].isin(list(val_data["PATIENT"]))
                    ]
                    train_data.reset_index(inplace=True, drop=True)
                    val_data.reset_index(inplace=True, drop=True)
                    df = pd.DataFrame(
                        {
                            "train": pd.Series(train_data["PATIENT"]),
                            "test": pd.Series([]),
                            "val": pd.Series(val_data["PATIENT"]),
                        }
                    )
                    df.to_csv(
                        os.path.join(args.split_dir, "TrainSplit.csv"), index=False
                    )
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                        from_id=False,
                        csv_path=os.path.join(args.split_dir, "TrainSplit.csv"),
                    )
                else:
                    df = pd.DataFrame(
                        {
                    ...
                    df.to_csv(
                        os.path.join(args.split_dir, "TrainValSplit.csv"), index=False
                    )
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                        from_id=False,
                        csv_path=os.path.join(args.split_dir, "TrainValSplit.csv"),
                    )

                datasets = (train_dataset, val_dataset, test_dataset)
                model, _, _ = Train_MIL_CLAM(
                    datasets=datasets, fold="FULL", args=args, trainFull=True
                )
                torch.save(
                    model.state_dict(),
                    os.path.join(args.projectFolder, "RESULTS", "finalModel"),
                )
                print()
                print("-" * 30)
                reportFile.close()
                ...
                folds = args.k
                kf = StratifiedKFold(
                    n_splits=folds, random_state=args.seed, shuffle=True
                )
                ...
                for train_index, test_index in kf.split(patientID, yTrue):
                    ...
                    print("FOR TRAIN SET...\n")
                    train_data = pd.DataFrame(
                        list(zip(trainPatients, trainyTrue, trainyTrueLabel)),
                        columns=["PATIENT", "yTrue", "yTrueLabel"],
                    )
                    ...
                    test_data = pd.DataFrame(
                        list(zip(testPatients, testyTrue, testyTrueLabel)),
                        columns=["PATIENT", "yTrue", "yTrueLabel"],
                    )
                    train_data.reset_index(inplace=True, drop=True)
                    ...
                    df = pd.DataFrame(
                        {
                            "train": pd.Series(train_data["PATIENT"]),
                            "test": pd.Series(test_data["PATIENT"]),
                            "val": pd.Series(val_data["PATIENT"]),
                        }
                    )
                    df.to_csv(
                        os.path.join(
                            args.split_dir,
                            "TrainTestValSplit_{}.csv".format(foldcounter),
                        ),
                        index=False,
                    )
                    train_dataset, val_dataset, test_dataset = dataset.Return_splits(
                    ...
                    model, results, test_auc = Train_MIL_CLAM(
                        datasets=datasets, fold=foldcounter, args=args, trainFull=False
                    )
                    ...
                    for item in list(results.keys()):
                        ...
                        yTrueLabe_test.append(
                            utils.get_key_from_value(
                                args.target_labelDict, temp["label"]
                            )
                        )
                        ...
                        for key in list(args.target_labelDict.keys()):
                            probs[key].append(
                                temp["prob"][0][
                                    utils.get_value_from_key(args.target_labelDict, key)
                                ]
                            )

                    probs = pd.DataFrame.from_dict(probs)
                    df = pd.DataFrame(
                        list(zip(patients, filaNames, yTrue_test, yTrueLabe_test)),
                        columns=["PATIENT", "FILENAME", "yTrue", "yTrueLabel"],
                    )
                    df = pd.concat([df, probs], axis=1)
                    testResultsPath = os.path.join(
                        args.result_dir,
                        "TEST_RESULT_SLIDE_BASED_FOLD_" + str(foldcounter) + ".csv",
                    )
                    df.to_csv(testResultsPath, index=False)
                    CalculatePatientWiseAUC(
                        resultCSVPath=testResultsPath,
                        args=args,
                        foldcounter=foldcounter,
                        clamMil=True,
                        reportFile=reportFile,
                    )
                    reportFile.write("-" * 30 + "\n")
                ...
                for i in range(args.k):
                    patientScoreFiles.append(
                        "TEST_RESULT_PATIENT_BASED_FOLD_" + str(i + 1) + ".csv"
                    )
                    slideScoreFiles.append(
                        "TEST_RESULT_SLIDE_BASED_FOLD_" + str(i + 1) + ".csv"
                    )

                CalculateTotalROC(
                    resultsPath=args.result_dir,
                    results=patientScoreFiles,
