        ...
        for pos in pos_list:
            if self.transform:
                if self.mag == "40x":  # 指定した領域(224 x 224)の画素を取得
                    img = svs.read_region(
                        (pos[0], pos[1]), 0, (b_size, b_size)
                    ).convert("RGB")
                elif self.mag == "20x":  # 指定した領域(448 x 448)
                    img = svs.read_region(
                        (pos[0] - (int(b_size / 2)), pos[1] - (int(b_size / 2))),
                        0,
                        (b_size * 2, b_size * 2),
                    ).convert("RGB")
                    ...
                    img = svs.read_region(
                        (
                            pos[0] - (int(b_size * 3 / 2)),
                            pos[1] - (int(b_size * 3 / 2)),
                        ),
                        1,
                        (b_size, b_size),
                    ).convert("RGB")
                    ...
                    img = svs.read_region(
                        (
                            pos[0] - (int(b_size * 7 / 2)),
                            pos[1] - (int(b_size * 7 / 2)),
                        ),
                        1,
                        (b_size * 2, b_size * 2),
                    ).convert("RGB")
