...
for slideID in train_all[40 * part : 40 * (part + 1)]:
    ...
    for h in range(b_h):
        for w in range(b_w):
            # サムネイル作成
            b_img = svs.read_region(
                (w * b_size, h * b_size), 0, (b_size, b_size)
            ).convert("RGB")
            ...
            if (
                statV.mean[0] < 230
                and np.count_nonzero(G_b > 230) < 224 * 224 / 2
                and statS.mean[0] > 0
                and np.mean(b_ratio) > 0.9
                and np.count_nonzero(b_ratio > 1) > 224 * 224 / 16
            ):
